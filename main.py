...
raid_timers_perm = {
    'dino': 259200,  # 72 hours
    'gele': 259200,
    'bt': 259200,
    'mord': 172800,  # 48 hours
    'hrung': 172800,
    'necro': 172800,
    'aggy': 230400, # 64 hours
    #'test': 30
}
            ...
            if content:
                ...
                # Check if the first word is a key in raid_timers
                    ...
                    if duration>raid_timers[content[0]]/2:
                        await message.channel.send(f"A raid timer for **{content[0]}** has started. Window will be open in <t:{int(expiry_time - raid_timers[content[0]]/2)}:f>(<t:{int(expiry_time - raid_timers[content[0]]/2)}:R>).")
                    else:
...
async def start_raid_timer(message, keyword, duration):
    ...
    start_time = time.time()
    running_timers[keyword] = (start_time, duration)
    await asyncio.sleep(duration - raid_timers[keyword]/2)  # Sleep until the timer reaches half
    if running_timers.get(keyword) == (start_time, duration):  # Check if the current timer is still running
        ...
        await asyncio.sleep(duration / 2)  # Sleep for the remaining half of the timer
...
async def display_running_timers(channel):
    ...
    for content, (start_time, duration) in sorted_timers:
        ...
        if content in raid_timers:
            if remaining_time > raid_timers[content] / 2:
                timer_list.append(f"* **{content}:** Window open <t:{int(start_time + duration - raid_timers[content]/2)}:R> - <t:{int(start_time + duration - raid_timers[content]/2)}:f>")
            else:
                timer_list.append(f"* **{content}:** Window closes <t:{int(start_time + duration)}:R> - <t:{int(start_time + duration)}:f>")
        elif content in event_timers_window:
            remaining_time_str = f"{int(hours)}:{int(minutes)}:{int(seconds)}"
            if event_flag[content] == 'open':
                timer_list.append(f"* **{content}:** {remaining_time_str} remaining - Window opens <t:{int(start_time + duration)}:R> - <t:{int(start_time + duration)}:f>")                        
            else:
                timer_list.append(f"* **{content}:** {remaining_time_str} remaining - Window closes <t:{int(start_time + duration)}:R> - <t:{int(start_time + duration)}:f>")
        elif content in event_timers:
            remaining_time_str = f"{int(hours)}:{int(minutes)}:{int(seconds)}"
            timer_list.append(f"* **{content}:** {remaining_time_str} remaining - Spawns <t:{int(start_time + duration)}:R> - <t:{int(start_time + duration)}:f>")
        else:
            if remaining_time>0:
                remaining_time_str = f"{int(hours)}:{int(minutes)}:{int(seconds)}"
                timer_list.append(f"* **{content}:** {remaining_time_str} remaining - Due <t:{int(start_time + duration)}:R> - <t:{int(start_time + duration)}:f>")
            else:
                remaining_time = time.time() - int(start_time + duration)
                hours, remainder = divmod(remaining_time, 3600)
                minutes, seconds = divmod(remainder, 60)
                remaining_time_str = f"{int(hours)}:{int(minutes)}:{int(seconds)}"
                timer_list.append(f"* **{content}:** {remaining_time_str} ago - Due <t:{int(start_time + duration)}:R> - **MISSING TIMER**")
    timer_list_str = '\n'.join(timer_list)
